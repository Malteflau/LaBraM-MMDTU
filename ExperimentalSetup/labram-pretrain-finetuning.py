...
def train_one_epoch(model, vqnsp, dataloader, optimizer, device, epoch, loss_scaler, max_norm, log_writer, 
    ...
    input_chans = list(range(model.pos_embed.shape[1]))  # Use all available channels
...
def main(args):
    ...
    # In the main function, update the model loading code
 # Import this at the top of your file
    import torch.serialization

    # In your model loading code:
    try:
        # Allow numpy.core.multiarray.scalar in safe globals
        torch.serialization.add_safe_globals([numpy.core.multiarray.scalar])
        
        # Get the model class
        model_class = getattr(modeling_pretrain, args.model)
        
        # Model kwargs with only necessary parameters
        model_kwargs = {}
        
        # Now instantiate the model with just those kwargs
        ...
        if args.pretrained and args.model_path:
            ...
            # Load with weights_only=False since we trust the file
